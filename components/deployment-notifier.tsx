...
export function DeploymentNotifier() {
  ...
  useEffect(() => {
    ...
    // First load — record the current deployment silently, don't toast
    ...
    // User dismissed — skip until a genuinely new deployment arrives
    if (isDismissed) {
      setLastSeenDeploymentId(deployment.deploymentId);
  ...
  }, [deployment, lastSeenDeploymentId, isDismissed]);
